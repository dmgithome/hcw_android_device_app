...
import com.hv.cabinet.ui.components.LocationChoiceStrip
...
fun TakeScreen(
    ...
    LaunchedEffect(active) {
        if (active) {
            PerfMonitor.mark("take_screen_interactive")
            PerfMonitor.measure(
                start = "home_card_take_pointerdown",
                end = "take_screen_interactive",
                label = "home_take_to_interactive"
            )
        } else {
    ...
    val submitEnabled = state.canSubmit && state.targetLocationId.isNotBlank()
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        AppScaffold(
            ...
            Column(
                ...
                // Location strip
                LocationChoiceStrip(
                    options = state.targetLocationOptions,
                    selectedId = state.targetLocationId,
                    loading = state.loadingTargetLocations,
                    onSelect = viewModel::updateTargetLocation
                )
